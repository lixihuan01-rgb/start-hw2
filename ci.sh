...
# Run graded tests
tests_failed=0
# Run assignment with sample input
timeout 30 build/main < input/inp01.txt > output/out01.txt || exit 1
timeout 30 build/main < input/inp02.txt > output/out02.txt || exit 1
timeout 30 build/main < input/inp03.txt > output/out03.txt || exit 1
timeout 30 build/main < input/inp04.txt > output/out04.txt || exit 1
timeout 30 build/main < input/inp05.txt > output/out05.txt || exit 1
echo "Running graded tests..."
timeout 10 build/tests/tests [graded] -r=github
if [ "${PIPESTATUS[0]}" -ne "0" ] ; then
	tests_failed=1
fi
...
echo -e "\n## clang-tidy Warnings (any warnings can result in a deduction)" >> ${GITHUB_STEP_SUMMARY}
